...
namespace Assets.Scripts.Player.States.Movement.States
{
    public class PlayerMovementState : IState
    {
        ...
        {
            if (
                !stateMachine.ReusableData.IsGrounded
                && stateMachine.CurrentState is not PlayerAirborneState
            )
            {
        ...
        {
            stateMachine.ReusableData.MovementVector =
                stateMachine.Player.MyInputMap.GroundedInputs.Move.ReadValue<Vector2>();
        }
        ...
        private bool IsGrounded()
        {
            ...
            Vector3 PlayerToGroundOffset =
                stateMachine.Player.transform.position + Vector3.up * FLOOR_RAYCAST_OFFSET;
            return Physics.Raycast(
                PlayerToGroundOffset,
                Vector3.down,
                out RaycastHit hit,
                MAX_TO_FLOOR_CAST,
                mask
        ...
        {
            return new Vector3(
                stateMachine.ReusableData.MovementVector.x,
                0f,
                stateMachine.ReusableData.MovementVector.y
            );
        }
        ...
        {
            stateMachine.Player.MyRigidbody.rotation = Quaternion.Lerp(
        ...
        {
            stateMachine.Player.MyRigidbody.rotation = Quaternion.Lerp(
                stateMachine.Player.gameObject.transform.rotation,
                targetRotation,
                Time.deltaTime * 6f
            );
        }
